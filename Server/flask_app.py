...
from classification.classification import init_model, run_pipeline
    ...
    if request.is_json:
        ...
        for anything in range(len(req)):
            ...
            # else:
                ...
                # k += 1
                # if k == 10:
                #     prediction, sat_counter = run_pipeline(Model, x, sat_counter)
                #     k = 0
...
def download():
    ...
    def generate(username,token):
        ...
        # write header of CSV
        w.writerow(('Timestamp', 'User', 'Accel_X', 'Accel_Y', 'Accel_Z', 'Gyro_X', 'Gyro_Y', 'Gyro_Z', 'Heart_rate', 'Token', 'Label','Prediction'))
        yield data.getvalue()
        data.seek(0)
        data.truncate(0)
